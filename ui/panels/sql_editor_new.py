...
from ui.components.ai_prompt import AIPromptDialog, AIContextMenu
...
class SQLEditorPanel(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.context_menu = AIContextMenu(self, self.db_manager, self.ai_integration)
        ...
        ttk.Button(btn_frame2, text="🤖 AI Prompt", command=self.show_ai_prompt).pack(side=tk.LEFT, padx=5)
    ...
    def show_context_menu(self, event):
        ...
        if self.context_menu:
            self.context_menu.show_context_menu(event)
    
    def show_ai_prompt(self):
        """Show the AI prompt dialog."""
        if not self.ai_prompt_dialog:
            self.ai_prompt_dialog = AIPromptDialog(self, self.db_manager, self.on_ai_generate)
        self.ai_prompt_dialog.show()
    
    def on_ai_generate(self, prompt):
        """Handle AI generation and insert result into editor."""
        if self.ai_integration:
            # Generate SQL using AI
            generated_sql = self.ai_integration.generate_sql_query(prompt)
            if generated_sql:
                # Clear editor and insert generated SQL
                self.editor.delete("1.0", tk.END)
                ...
                if self.results_viewer:
                    self.results_viewer.display_error("Failed to generate SQL with AI.")
        else:
            if self.results_viewer:
                self.results_viewer.display_error("AI integration not available. API key may not be set.")
